    ...
    outputs: [
      {
        ...
        components: [
          ...
          { name: "subtaskCount", type: "uint256" },
...
export class OnChainService {
  ...
  constructor() {
    ...
    this.orchestratorAccount = privateKeyToAccount(
    ...
    console.log(
      "✓ OnChainService ready:",
      this.orchestratorAccount.address
    );
  }
  ...
  private getWalletClient() {
    if (!this.walletClient) {
      throw new Error(
        "OnChainService: DEPLOYER_PRIVATE_KEY not configured in .env"
      );
    }
  ...
  async registerAgent(params: {
    ...
    const capHashes = params.capabilities.map(
      (c) => keccak256(toBytes(c)) as `0x${string}`
    );
  ...
  async approveEscrow(params: { amount: number }): Promise<string> {
    const hash = await this.getWalletClient().writeContract({
      ...
      args: [
        addresses.contracts.OrchestratorEscrow as `0x${string}`,
  ...
  async createTask(params: {
    ...
    const taskId = log?.topics[1]
      ? parseInt(log.topics[1], 16)
      : Date.now();

    return { taskId, txHash: hash };
  }

  async assignSubtask(params: {
    taskId: number;
